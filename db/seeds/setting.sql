...
-- DROP TABLE setting.setting_business_partner_fuel_order_issuer;
CREATE TABLE setting.setting_business_partner_fuel_order_issuer (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1 INCREMENT BY 1),
    name VARCHAR(100) NOT NULL,
    ...
    modified_date TIMESTAMP,
    is_active BOOLEAN NOT NULL,
    CONSTRAINT pk_setting_business_partner_fuel_order_issuer_id PRIMARY KEY (id),
    CONSTRAINT fk_setting_business_partner_fuel_order_issuer_created_by_user FOREIGN KEY (created_by) REFERENCES auth.auth_user(id),
    CONSTRAINT fk_setting_business_partner_fuel_order_issuer_modified_by_user FOREIGN KEY (modified_by) REFERENCES auth.auth_user(id)
);
