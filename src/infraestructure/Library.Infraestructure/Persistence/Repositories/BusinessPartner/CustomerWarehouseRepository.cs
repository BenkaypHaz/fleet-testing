...
namespace Library.Infraestructure.Persistence.Repositories.Customer
{
    public class CustomerWarehouseRepository
    {
        ...
        public async Task<GenericResponseHandler<List<CustomerWarehouseReadDto>>> GetBySearch(string? searchValue)
        {
                ...
                if (!string.IsNullOrEmpty(searchValue))
                {
                    ...
                    query = query.Where(x =>
                        ...
                        x.Code.ToString().StartsWith(searchValue) ||
                        x.Code.ToString().Equals(searchValue));
                }
